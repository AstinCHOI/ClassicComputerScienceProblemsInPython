...
if __name__ == "__main__":
    albums: List[Album] = [Album("Got to Be There", 1972, 35.45, 10), Album("Ben", 1972, 31.31, 10),
                           Album("Music & Me", 1973, 32.09, 10), Album(
                               "Forever, Michael", 1975, 33.36, 10),
                           Album("Off the Wall", 1979, 42.28, 10), Album(
                               "Thriller", 1982, 42.19, 9),
                           Album("Bad", 1987, 48.16, 10), Album(
                               "Dangerous", 1991, 77.03, 14),
    ...
    for index, cluster in enumerate(clusters):
        print(
            f"군집 {index} 평균 길이 {cluster.centroid.dimensions[0]} 평균 트랙 {cluster.centroid.dimensions[1]}: {cluster.points}\n")
